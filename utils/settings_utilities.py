...
class SettingUtilities:
    ...
    def _initialize_paths(self):
        ...
        self.face_images_folder_path = os.path.join(
            self.temp_folder_path, 'face_images'
        )
        ...
        self.face_video_folder_path = os.path.join(
            self.video_folder_path, 'face_videos'
        )

        self.adjusted_video_folder_path = os.path.join(
            self.video_folder_path, 'adjusted_video'
        )
        self.adjusted_input_video_folder_path = self.input_video_path
        self.adjusted_video_output_folder_path = os.path.join(
            self.adjusted_video_folder_path, 'output'
        )
        self.adjusted_face_video_input_path = os.path.join(
            self.adjusted_input_video_folder_path, 'input_video.mp4'
        )
        self.adjusted_face_video_output_path = os.path.join(
            self.adjusted_video_output_folder_path, 'video.mp4'
        )

        self.split_video_folder_path = os.path.join(
            self.face_video_folder_path, 'split_videos'
        )
        self.split_video_input_folder_path = self.adjusted_video_output_folder_path
        self.split_video_output_folder_path = os.path.join(
            self.split_video_folder_path, 'output'
        )
        self.split_video_input_video_path = os.path.join(
            self.split_video_input_folder_path, 'video.mp4'
        )

        self.wav2lip_video_folder_path = os.path.join(
            self.face_video_folder_path, 'wav2lip_videos'
        )
        self.wav2lip_input_video_folder_path = self.split_video_folder_path
        self.wav2lip_output_video_folder_path = os.path.join(
            self.wav2lip_video_folder_path, 'wav2lip_output'
        )

        self.wav2lip_path = os.path.join(self.sub_modules_path, 'Wav2Lip')
        sys.path.append(self.wav2lip_path)
        ...
        self.wav2lip_checkpoint_wav2lip_path = os.path.join(
            self.wav2lip_checkpoints_path, self.wav2lip_checkpoint_wav2lip_name
        )
        self.wav2lip_checkpoint_wav2lip_gan_path = os.path.join(
            self.wav2lip_checkpoints_path, self.wav2lip_checkpoint_wav2lip_gan_name
        )

        self.overlay_video_folder_path = os.path.join(
            self.face_video_folder_path, 'overlay_videos'
        )
        self.overlay_input_video_folder_path = self.wav2lip_output_video_folder_path
        self.overlay_output_video_folder_path = os.path.join(
            self.overlay_video_folder_path, 'output'
        )

        self.blurdetection2_path = os.path.join(self.sub_modules_path, 'BlurDetection2')
        sys.path.append(self.blurdetection2_path)
        self.blurdetection2_video_folder_path = os.path.join(
            self.face_video_folder_path, 'blurdetection2_videos'
        )
        self.blurdetection2_input_video_path = self.overlay_output_video_folder_path
        self.blurdetection2_output_video_folder_path = os.path.join(
            self.blurdetection2_video_folder_path, 'output'
        )
        self.blurdetection2_model_name = 'model.pth'
        self.blurdetection2_model_folder_path = os.path.join(
            self.blurdetection2_path, 'models'
        )
        self.blurdetection2_model_path = os.path.join(
            self.blurdetection2_model_folder_path, self.blurdetection2_model_name
        )

        self.esrgan_path = os.path.join(self.sub_modules_path, 'ESRGAN')
        sys.path.append(self.esrgan_path)
        self.esrgan_inhagcutils_path = os.path.join(
            self.esrgan_path, 'inhagcutils.ipynb'
        )
        self.esrgan_video_folder_path = os.path.join(
            self.face_video_folder_path, 'esrgan_videos'
        )
        ...
        self.esrgan_dejpeg_dir_path = os.path.join(
            self.temp_folder_path, 'ESRGAN', 'dejpeg'
        )
        self.esrgan_tmp_dir_path = os.path.join(self.temp_folder_path, 'ESRGAN', 'tmp')
        self.esrgan_mask_dir_path = os.path.join(
            self.temp_folder_path, 'ESRGAN', 'tmp', 'mask'
        )
        self.esrgan_upscaled_dir_path = os.path.join(
            self.temp_folder_path, 'ESRGAN', 'upscaled'
        )

        self.esrgan_models_dir_path = os.path.join(self.esrgan_path, 'models')
        self.esrgan_model_path = os.path.join(
            self.esrgan_models_dir_path, self.esrgan_model_name
        )
        self.esrgan_model_dejpeg_path = os.path.join(
            self.esrgan_models_dir_path, self.esrgan_model_dejpeg_name
        )
        ...
        self.adjusted_audio_folder_path = os.path.join(
            self.audio_folder_path, 'adjusted'
        )

        self.wav2lip_input_silent_audio_path = os.path.join(
            self.silent_audio_folder_path, 'silent_audio_lipsync.wav'
        )
        self.wav2lip_input_audio_path = os.path.join(
            self.adjusted_audio_folder_path, 'adjusted_audio.wav'
        )
    ...
    def generate_dynamic_files(self):
        for i in range(self.number_of_faces):
            ...
            split_face_video_output_template = (
                self.split_face_video_output_template_path.format(i=i)
            )
            wav2lip_input_video_template = (
                self.wav2lip_input_video_template_path.format(i=i)
            )
            wav2lip_output_video_template = (
                self.wav2lip_output_video_template_path.format(i=i)
            )
            overlay_input_video_template = (
                self.overlay_input_video_template_path.format(i=i)
            )

            setattr(self, f'face_image_{i}_path', face_image_template)
            setattr(
                self,
                f'split_face_video_output_{i}_path',
                split_face_video_output_template,
            )
            setattr(self, f'wav2lip_input_video_{i}_path', wav2lip_input_video_template)
            setattr(
                self, f'wav2lip_output_video_{i}_path', wav2lip_output_video_template
            )
    ...
    def reset_to_default(self, attribute_name):
        if attribute_name in self.default_paths:
            ...
            raise AttributeError(
                f'No attribute named {attribute_name} found in the default paths'
            )
...
if __name__ == '__main__':
    ...
    settings.generate_dynamicfiles()
