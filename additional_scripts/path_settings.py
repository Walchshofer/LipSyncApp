...
class PathSettings:
    def __init__(self, number_of_faces=1):  # Set a default value for number_of_faces
        ...
        self.local_project_path = '/home/patrick_wsl/anaconda3/envs/LipSyncApp-py391/local_wsl_projects'
    ...
    def create_directories(self):
        directories = [
            self.work_data_path, self.input_folder_path, self.output_folder_path,
            self.temp_folder_path, self.face_images_folder_path, self.video_folder_path,
            self.face_video_folder_path, self.adjusted_video_folder_path,
            self.adjusted_input_video_folder_path, self.adjusted_video_output_folder_path,
            self.split_video_folder_path, self.split_video_input_folder_path,
            self.split_video_output_folder_path, self.wav2lip_video_folder_path,
            self.wav2lip_input_video_folder_path, self.wav2lip_output_video_folder_path,
            self.overlay_video_folder_path, self.overlay_input_video_folder_path,
            self.overlay_output_video_folder_path, self.blurdetection2_video_folder_path,
            self.blurdetection2_input_video_path, self.blurdetection2_output_video_folder_path,
            self.esrgan_video_folder_path, self.esrgan_input_video_folder_path,
            self.esrgan_output_video_dir_path, self.esrgan_dejpeg_dir_path,
            self.esrgan_tmp_dir_path, self.esrgan_mask_dir_path,
            self.esrgan_upscaled_dir_path, self.audio_folder_path,
            self.silent_audio_folder_path,
            self.adjusted_audio_folder_path, self.esrgan_audio_folder_path
        ]
...
if __name__ == '__main__':
    ...
    path_settings.create_directories()
    path_settings.generate_dynamic_files()